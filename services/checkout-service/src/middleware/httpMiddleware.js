...
const notFoundHandler = (req, res) => {
  res
    .status(404)
    .json(errorResponse("Route not found", "CHECKOUT_ROUTE_NOT_FOUND"));
};
...
const errorHandler = (error, req, res, next) => {
  ...
  const message = error.message || "Checkout service failed to process request";
  const code = error.code || "CHECKOUT_INTERNAL_ERROR";
