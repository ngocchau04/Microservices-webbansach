...
| P1-04 | `Backend/test/cartController.unit.test.js` lines 201, 290, 401, 550 | Mock leak risk | Chuyển sang `jest.spyOn` + `afterEach(() => jest.restoreAllMocks())` |
| P1-05 | `Backend/test/authorizationService.unit.test.js` line 172 | Mock leak risk | Chuyển `jwt.verify = jest.fn()` sang `jest.spyOn(jwt, "verify")` |
| P1-06 | `Backend/test/searchController.unit.test.js` lines 102, 374, 438 | Mock leak risk | Cùng fix pattern như P1-04 |
| P1-07 | `Backend/test/orderController.unit.test.js` line 280 | Mock restore không có try/finally | Thêm try/finally |
...
| # | File | Vấn đề | Hành động |
|---|---|---|---|
| P2-01 | `services/identity-service/test/*.test.js` (3 files) | `\|\|` pattern trong token assertion | Fix API response shape để nhất quán; xóa `\|\|` |
...
| P3-06 | `Backend/test/orderController.unit.test.js` line 248 | `_doc` internal property | Thay bằng `.toObject()` |
| P3-07 | `Backend/test/searchController.unit.test.js` lines 34-35 | `console.log` còn sót | Xóa debug logs |
